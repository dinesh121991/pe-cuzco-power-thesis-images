Wait<-function(F1,F2,F3,F4,F5,F6)
{
	...
	postscript("cdf-exec-SLURM-1024-16384.eps",width=8,height=5,onefile = TRUE, horizontal = FALSE,paper="special")
#	png(file="Cumulated Distribution function on Wait time(OAR-SLURM).png",height=850,width=1000)
	
	plot(ecdf(sort(Wait1)),pch=22, xlim=c(0,max(Wait1,Wait2,Wait3,Wait4,Wait5,Wait6)),xlab="Execution time [s]",ylab="Jobs [%]", main= "CDF on Execution time for Light-ESP benchmark (230 jobs)\nupon a 16 cpu/node cluster with variation on the number of nodes \n (emulation with 400 physical nodes)")

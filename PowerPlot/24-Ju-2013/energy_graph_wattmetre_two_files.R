...
#        png(file="Power_consumption_wattmeter.png",height=1000,width=1000)
	postscript("cr_crl_29_ju_ex-3-width-20.eps",width=20,height=10,onefile = TRUE, horizontal = FALSE,paper="special")
...
#	axis(4,col="red",col.axis="red")
	...
	legend("topright",pch=22,col=c("blue","red"),c("cons_res","cons_res_layout_power"),inset=0.04)
	  legend("center",title="Type of collection - Calculated Energy Consumption",c(paste("cons_res - ",surf_f,"Joules"),paste("cons_res_layout_power - ",surf_p,"Joules"),col=c("blue","red"),lty=c(1,2),inset=0.001))
        dev.off()
}


ConsAllEnergy("cr_pl_jl_ex-2.compl.dat","crl_pl_jl-ex-3.compl.dat")

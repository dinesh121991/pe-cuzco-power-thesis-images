...
#	png(file="Cumulated Distribution function on Wait time(OAR-SLURM).png",height=850,width=1000)
	...
	plot(ecdf(sort(Wait1)),pch=22, xlim=c(0,max(Wait1,Wait2)),xlab="Submission time difference from start [s]",ylab="Jobs [%]", main= "CDF on Submission time for Light-ESP benchmark (230 jobs)\nupon a 16 cpu/node cluster with variation on the number of nodes")
...
#	mtext(paste("files :",F1, "-", F2)) 
	legend("bottomright",c("cons_res_no_overhead","cons_res_layout_no_overhead"),pch=c(22,20),lty=c(1,0),inset=0.02)
	dev.off()
...
#	dev.off()
	...
	png(file="Submit_time (comparaison).png",height=850,width=1000)
  ...
  #	colnames(m)<-h1$breaks[2:length(h1$breaks)]
	...
	mtext(paste("Files :",F1,"-",F2))	
  	...
  	legend("top",														# where to put the legend
  		...
  		c("cons_res","cons_res_layout"),	# what to write
